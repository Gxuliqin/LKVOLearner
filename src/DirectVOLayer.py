...
class DirectVO(nn.Module):
    ...
    def warp_batch_func(self, img_batch, inv_depth, level_idx, R_batch, t_batch):
        ...
        xyz = R_batch[:, :, 0:2].bmm(xy.view(1, 2, N).expand(batch_size, 2, N)) \
              ...
              + t_batch.view(batch_size, 3, 1).expand(batch_size, 3, N) * inv_depth.view(-1, 1, N).expand(batch_size, 3,
                                                                                                          N)
        ...
        u_warp = ((xy_warp[:, 0, :] * self.camparams['fx'] + self.camparams['cx']) -
                  getattr(self, 'x_' + str(level_idx))[0]).view(
            batch_size, N) / 2 ** level_idx
        v_warp = ((xy_warp[:, 1, :] * self.camparams['fy'] + self.camparams['cy']) -
                  getattr(self, 'y_' + str(level_idx))[0]).view(
            batch_size, N) / 2 ** level_idx

        Q, in_view_mask = grid_bilinear_sampling(img_batch, u_warp, v_warp)
        return Q, in_view_mask * (z.view_as(in_view_mask) > 1e-10).float()

    def compute_phtometric_loss(self, ref_frames_pyramid, src_frames_pyramid, ref_inv_depth_pyramid,
    ...
    def update(self, frames_pyramid, max_itr_num=10):
        ...
        for level_idx in range(self.pyramid_layer_num - 1, -1, -1):
            ...
            for i in range(max_itr_num):
                # print(i)
                # cur_time = timer()
                pixel_warp, in_view_mask = self.warp_batch(frames_pyramid[level_idx], level_idx, rot_mat_batch,
                                                           trans_batch)
                # t_warp += timer()-cur_time

                temporal_grad = pixel_warp - self.ref_frame_pyramid[level_idx].view(3, -1).unsqueeze(0).expand_as(
                    pixel_warp)

                photometric_cost, min_perc_in_view = compute_photometric_cost_norm(temporal_grad.data,
                                                                                   in_view_mask.data)
                ...
                if (photometric_cost < max_photometric_cost).max() > 0:
                    ...
                    trans_batch_new = d_rot_mat_batch.bmm(trans_batch.view(frame_num, 3, 1)).view(frame_num, 3) - dp[:,
                                                                                                                  3:6]
