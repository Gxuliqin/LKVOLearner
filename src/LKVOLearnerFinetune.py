...
class LKVOLearner(nn.Module):
    ...
    def forward(self, frames, camparams, max_lk_iter_num=10, lk_level=1):
        cost, photometric_cost, smoothness_cost, ref_frame, ref_inv_depth \
            = self.lkvo.forward(frames, camparams, self.ref_frame_idx, self.lambda_S, max_lk_iter_num=max_lk_iter_num,
...
class LKVOKernel(nn.Module):
    ...
    def forward(self, frames, camparams, ref_frame_idx, lambda_S=.5, do_data_augment=True, use_ssim=True,
        ...
        rot_mat_batch, trans_batch = self.vo.update_with_init_pose(src_frames_pyramid[0:lk_level],
                                                                   max_itr_num=max_lk_iter_num,
                                                                   rot_mat_batch=rot_mat_batch, trans_batch=trans_batch)

        photometric_cost = self.vo.compute_phtometric_loss(self.vo.ref_frame_pyramid, src_frames_pyramid,
                                                           ref_inv_depth_pyramid, src_inv_depth_pyramid, rot_mat_batch,
                                                           trans_batch, levels=[0, 1, 2, 3], use_ssim=use_ssim)
        smoothness_cost = self.vo.multi_scale_image_aware_smoothness_cost(inv_depth0_pyramid, frames_pyramid,
                                                                          levels=[2, 3], type=self.smooth_term) \
                          + self.vo.multi_scale_image_aware_smoothness_cost(inv_depth_norm_pyramid, frames_pyramid,
                                                                            levels=[2, 3], type=self.smooth_term)

        cost = photometric_cost + lambda_S * smoothness_cost
        return cost, photometric_cost, smoothness_cost, self.vo.ref_frame_pyramid[0], ref_inv_depth0_pyramid[
            0] * inv_depth_mean_ten
