...
if __name__ == '__main__':
    ...
    sfmlearner = SfMLearner(img_size=img_size, ref_frame_idx=1, lambda_S=opt.lambda_S, gpu_ids=gpu_ids,
        ...
        for ii, data in enumerate(dataloader):
            ...
            if np.mod(step_num, opt.print_freq) == 0:
                ...
                print('%s: %s / %s, ... elapsed time: %f (s)' % (
                    epoch, step_num, int(len(dataset) / opt.batchSize), elapsed_time))
